#include <bits/stdc++.h>
using namespace std;

// ANSI Color Codes (works in most modern terminals)
#define RESET   "\033[0m"
#define BOLD    "\033[1m"
#define RED     "\033[31m"
#define GREEN   "\033[32m"
#define YELLOW  "\033[33m"
#define BLUE    "\033[34m"
#define CYAN    "\033[36m"
#define WHITE   "\033[37m"

class Packet {
public:
    int id;
    int source;
    int destination;
    string data;
    double size;

    Packet(int i, int s, int d, string dat) 
        : id(i), source(s), destination(d), data(dat), size(512.0) {}

    void display() const {
        cout << GREEN << "Packet " << id << " created: " << data << RESET << endl;
    }
};

class Node {
public:
    int id;
    string name;
    Node(int i, string n = "") : id(i), name(n) {}
};

class Network {
private:
    vector<Node> nodes;
    vector<vector<pair<int, double>>> adj;
    int packetCounter = 0;

public:
    Network() {}

    void addNode(string name = "") {
        int id = nodes.size();
        nodes.emplace_back(id, name.empty() ? "Node" + to_string(id) : name);
        adj.emplace_back();
        cout << GREEN << "✓ Node " << id << " (" << nodes[id].name << ") added successfully." << RESET << endl;
    }

    void addConnection(int u, int v, double delay) {
        if (u < 0 || u >= nodes.size() || v < 0 || v >= nodes.size() || u == v || delay <= 0) {
            cout << RED << "✗ Invalid connection parameters!" << RESET << endl;
            return;
        }
        adj[u].emplace_back(v, delay);
        adj[v].emplace_back(u, delay);
        cout << GREEN << "✓ Connection added: " << u << " <-> " << v 
             << " (delay = " << delay << "s)" << RESET << endl;
    }

    vector<int> findShortestPath(int src, int dest) {
        if (src == dest) return {src};
        
        vector<int> parent(nodes.size(), -1);
        vector<bool> visited(nodes.size(), false);
        queue<int> q;

        q.push(src);
        visited[src] = true;

        while (!q.empty()) {
            int curr = q.front(); q.pop();
            for (auto& p : adj[curr]) {
                int nei = p.first;
                if (!visited[nei]) {
                    visited[nei] = true;
                    parent[nei] = curr;
                    q.push(nei);
                    if (nei == dest) goto reconstruct;
                }
            }
        }

    reconstruct:
        vector<int> path;
        if (parent[dest] == -1 && src != dest) return path;

        for (int at = dest; at != -1; at = parent[at]) {
            path.push_back(at);
        }
        reverse(path.begin(), path.end());
        return path;
    }

    void sendPacket(int src, int dest, string data) {
        int pid = packetCounter++;
        Packet pkt(pid, src, dest, data);
        pkt.display();

        vector<int> path = findShortestPath(src, dest);
        if (path.empty()) {
            cout << RED << "✗ No path exists from Node " << src << " to Node " << dest << "!" << RESET << "\n\n";
            return;
        }

        cout << CYAN << "Path taken: ";
        for (size_t i = 0; i < path.size(); ++i) {
            cout << path[i];
            if (i < path.size() - 1) cout << " → ";
        }
        cout << RESET << endl;

        double currentTime = 0.0;
        for (size_t i = 0; i < path.size() - 1; ++i) {
            int from = path[i];
            int to = path[i + 1];

            double delay = 0.0;
            for (auto& p : adj[from]) {
                if (p.first == to) {
                    delay = p.second;
                    break;
                }
            }

            currentTime += delay;
            cout << YELLOW << "   Time " << fixed << setprecision(1) 
                 << currentTime << "s: " << RESET
                 << "Packet moved from Node " << from 
                 << " → Node " << to 
                 << " (delay: " << delay << "s)" << endl;
        }

        cout << GREEN << "✓ Packet " << pid << " successfully delivered to Node " 
             << dest << " at time " << currentTime << "s!" << RESET << "\n\n";
    }

    void displayNetwork() const {
        cout << "\n" << BLUE << "╔════════════════════════════════════════════════════════════╗" << RESET << endl;
        cout << BLUE << "║" << RESET << "                NETWORK TOPOLOGY (" 
             << nodes.size() << " nodes)                    " 
             << BLUE << "║" << RESET << endl;
        cout << BLUE << "╠════════════════════════════════════════════════════════════╣" << RESET << endl;

        for (int i = 0; i < nodes.size(); ++i) {
            cout << BLUE << "║ " << RESET << "Node " << setw(2) << i 
                 << " (" << nodes[i].name << ") → ";
            if (adj[i].empty()) {
                cout << "no connections";
            } else {
                for (auto& p : adj[i]) {
                    cout << p.first << "(d=" << p.second << "s) ";
                }
            }
            cout << string(50 - (to_string(i).length() + nodes[i].name.length() + 20), ' ') 
                 << BLUE << "║" << RESET << endl;
        }
        cout << BLUE << "╚════════════════════════════════════════════════════════════╝" << RESET << "\n" << endl;
    }
};

// Function to print a nice header
void printHeader() {
    cout << "\n";
    cout << BLUE << "╔════════════════════════════════════════════════════════════╗" << RESET << endl;
    cout << BLUE << "║" << RESET << "          " << BOLD << "NETWORK PACKET SIMULATOR" << RESET 
         << "          " << BLUE << "║" << RESET << endl;
    cout << BLUE << "║" << RESET << "               Object-Oriented C++ Project               " 
         << BLUE << "║" << RESET << endl;
    cout << BLUE << "╚════════════════════════════════════════════════════════════╝" << RESET << "\n" << endl;
}

// Function to print main menu with box
void printMenu() {
    cout << CYAN << "╔════════════════════════════════════════════════════════════╗" << RESET << endl;
    cout << CYAN << "║" << RESET << "                        MAIN MENU                        " 
         << CYAN << "║" << RESET << endl;
    cout << CYAN << "╠════════════════════════════════════════════════════════════╣" << RESET << endl;
    cout << CYAN << "║" << RESET << "  1. Add New Node                                      " 
         << CYAN << "║" << RESET << endl;
    cout << CYAN << "║" << RESET << "  2. Add Connection (with delay)                       " 
         << CYAN << "║" << RESET << endl;
    cout << CYAN << "║" << RESET << "  3. Send Packet                                       " 
         << CYAN << "║" << RESET << endl;
    cout << CYAN << "║" << RESET << "  4. Display Network Topology                          " 
         << CYAN << "║" << RESET << endl;
    cout << CYAN << "║" << RESET << "  5. Exit Simulator                                    " 
         << CYAN << "║" << RESET << endl;
    cout << CYAN << "╚════════════════════════════════════════════════════════════╝" << RESET << "\n";
    cout << "Enter your choice (1-5): ";
}

int main() {
    Network net;

    printHeader();

    // Default example network
    cout << YELLOW << "Creating default example network..." << RESET << endl;
    for (int i = 0; i < 6; ++i) net.addNode();
    net.addConnection(0, 1, 1.2);
    net.addConnection(1, 2, 0.8);
    net.addConnection(0, 3, 2.5);
    net.addConnection(3, 4, 1.5);
    net.addConnection(2, 4, 3.0);
    net.addConnection(4, 5, 1.0);
    net.addConnection(1, 5, 4.0);

    net.displayNetwork();

    int choice;
    do {
        printMenu();
        cin >> choice;

        if (choice == 1) {
            string name;
            cout << "Enter node name (optional, press Enter for default): ";
            cin.ignore();
            getline(cin, name);
            net.addNode(name);
        } 
        else if (choice == 2) {
            int u, v;
            double delay;
            cout << "Enter Node U, Node V, Delay (in seconds): ";
            cin >> u >> v >> delay;
            net.addConnection(u, v, delay);
        } 
        else if (choice == 3) {
            int src, dest;
            string data;
            cout << "Enter Source Node ID: ";
            cin >> src;
            cout << "Enter Destination Node ID: ";
            cin >> dest;
            cout << "Enter packet message/content: ";
            cin.ignore();
            getline(cin, data);
            net.sendPacket(src, dest, data);
        } 
        else if (choice == 4) {
            net.displayNetwork();
        } 
        else if (choice == 5) {
            cout << GREEN << "\nThank you for using the Network Packet Simulator!" << RESET << endl;
            cout << "Goodbye! 👋\n" << endl;
        } 
        else {
            cout << RED << "✗ Invalid choice! Please enter a number between 1 and 5." << RESET << endl;
        }

        cout << "\nPress Enter to continue...";
        cin.ignore(numeric_limits<streamsize>::max(), '\n');
        cin.get();   // Wait for user to press Enter
    } while (choice != 5);

    return 0;
}