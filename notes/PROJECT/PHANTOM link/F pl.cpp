...
public:
    ...
    static void loadHistory(string user, string other) {
        vector<string> paths = {
            "data/chats/" + user + "_" + other + "/",
            "data/chats/" + other + "_" + user + "/"
        };
        ...
        for (auto &p : paths) {
            ...
            for (auto &file : fs::directory_iterator(p)) {
                ...
                seen.insert(file.path().string());
            }
        }

        sort(all.begin(), all.end(), [](Message a, Message b) {
